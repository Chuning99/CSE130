...
digitsOfInt n =
  if n > 0
    then digitsOfInt (n `div` 10) ++ [n `mod` 10]
    else []
...
-- sumList([9,8,7,6]) = 30, presistence = 1
-- sumList([3]), presistence = 2

additivePersistence :: Int -> Int
additivePersistence n =
  if (length (digitsOfInt n)) == 1
    then 0
...
--  digitalRoot(sumList([9,8,7,6])) == digitalRoot(30)
--  digitalRoot(sumList([3,0])) = digitalRoot(3)
--  length = 1 , return 3;
