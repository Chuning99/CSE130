...
eval env (EBin operator e1 e2) = evalOp operator (eval env e1) (eval env e2)
eval env (EIf e1 e2 e3)
  | value = eval env e2
  | not value = eval env e3
  | otherwise = throw (Error "type error")
  where
    (VBool value) = eval env e1
eval env (EVar x) = lookupId x env
eval _ ENil = VNil
eval env (ELet id e1 e2) = eval ((id, eval env e1) : env) e2
eval env (ELam x e) = VClos env x e
eval env (EApp fun argu) =
          case (eval env fun) of 
                                    VPrim f -> f (eval env argu)
                                    VClos funEnv x exp -> eval env' exp
                                          where
                                          value = eval env argu
                                          eenv = (x,value) : funEnv
                                          env' = (eenv) ++ env
...
evalOp Eq (VCons x y) (VCons m n)
      | firP  = evalOp Eq y n
      | otherwise = VBool False
          where  (VBool firP) = evalOp Eq x m
...
prelude :: Env
prelude =
  [ ("head", VPrim (\(VCons head _) -> head)),
    ("tail", VPrim (\(VCons _ tail) -> tail))
